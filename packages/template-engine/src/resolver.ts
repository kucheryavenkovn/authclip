...
export type SendMessageFn = (tabId: number, message: any) => Promise<any>;

let _sendMessage: SendMessageFn | null = null;

export function setMessageSender(fn: SendMessageFn): void {
	_sendMessage = fn;
}

async function defaultSendExtractContent(tabId: number, selector: string, attribute?: string, extractHtml?: boolean): Promise<any> {
	if (!_sendMessage) {
		console.error('No message sender configured for selector resolution');
		return undefined;
	}
	return _sendMessage(tabId, {
...
async function resolveSelectorVariable(selectorExpr: string, tabId?: number): Promise<any> {
	...
	try {
		const response = await defaultSendExtractContent(tabId, selector.replace(/\\"/g, '"'), attribute, extractHtml);
